...
(function () {
  ...
  function render() {
    ...
    /* The retailer's category list — js/ra-categories.js holds the one copy. This page used to
    ...
    var catOpts = cats.map(function(c){ return '<option' + (c===PROD.cat?' selected':'') + '>' + esc(c) + '</option>'; }).join('');
    /* a product may still carry a category that is no longer in the list */
