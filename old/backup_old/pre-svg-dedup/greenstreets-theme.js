...
(function(){
  ...
  function buildSelect(sel){
    ...
    function positionMenu(){
      /* the menu is position:fixed (a portal) so an ancestor's overflow:hidden can never clip it — e.g. a
         "Rows per page" select at the bottom of a `.grp` card. Anchor it to the trigger, flipping up if needed. */
    ...
    function open(){ if(sel.disabled)return; render(); wrap.classList.add('open'); positionMenu(); }
  ...
  function gsAddBulk(table){
    ...
    var all=document.createElement('input'); all.type='checkbox'; all.title='Select all'; th.appendChild(all);
    ...
    all.addEventListener('change',function(){
      Array.prototype.forEach.call(table.tBodies[0].rows,function(r){ if(r.style.display==='none'||r.classList.contains('gs-empty-row'))return; var c=r.querySelector('.gs-row-check'); if(c)c.checked=all.checked; });
      refresh();
    });
    function refresh(){
      var vis=0, sel=0;
      Array.prototype.forEach.call(table.tBodies[0].rows,function(r){ if(r.style.display==='none'||r.classList.contains('gs-empty-row'))return; var c=r.querySelector('.gs-row-check'); if(!c)return; vis++; if(c.checked)sel++; });
